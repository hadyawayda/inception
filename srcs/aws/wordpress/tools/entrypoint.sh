...
log()     { echo -e "${CYAN}[WP-ENTRYPOINT]${RESET} $1" ; }
...
if ! wp core is-installed --path="$WP_PATH" --allow-root; then
    ...
    wp core install --path="$WP_PATH" --url="https://${DOMAIN_NAME}" --title="$WP_TITLE" --admin_user="$WP_ADMIN_USER" --admin_password="$ADMIN_PASS" --admin_email="$WP_ADMIN_EMAIL" --skip-email --allow-root
...
wp user get "$WP_USER" --path="$WP_PATH" --allow-root >/dev/null 2>&1 || wp user create "$WP_USER" "$WP_USER_EMAIL" --user_pass="$USER_PASS" --role=author --path="$WP_PATH" --allow-root
