#!/bin/bash

# Check if MariaDB has been initialized
if [ ! -d /var/lib/mysql/mysql ]; then
    echo "Initializing MariaDB database..."
    mysqld --initialize-insecure
    mysqld &
    sleep 10

    # Set root password
    if [ -f "$MARIADB_ROOT_PASSWORD_FILE" ]; then
        mysql -e "SET PASSWORD FOR 'root'@'localhost' = PASSWORD('$(cat $MARIADB_ROOT_PASSWORD_FILE)');"
    else
        echo "Root password not set, skipping root password configuration"
    fi

    # Create the WordPress user
    if [ -f "$MARIADB_PASSWORD_FILE" ]; then
        mysql -e "CREATE USER '${MARIADB_USER}'@'%' IDENTIFIED BY '$(cat $MARIADB_PASSWORD_FILE)';"
        mysql -e "GRANT ALL PRIVILEGES ON *.* TO '${MARIADB_USER}'@'%';"
        mysql -e "FLUSH PRIVILEGES;"
    else
        echo "WordPress password not set, skipping user creation"
    fi
    
    mysqladmin -u root -p$(cat $MARIADB_ROOT_PASSWORD_FILE) shutdown
    sleep 5
fi

# Run MariaDB
exec mysqld
